...
B14_SCRIPT="${REPO_ROOT}/scripts/apply-candidate-b14-mipmaps-tail-fusion.py"
python3 "${REPO_ROOT}/scripts/check-mipmaps-device-agnostic.py" "${B14_SCRIPT}"
python3 "${B14_SCRIPT}" --root "${REPO_ROOT}"

# Apply CPU hot-path transforms last so earlier source transforms cannot
# invalidate their anchors.
