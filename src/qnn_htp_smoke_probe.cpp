...
struct QnnScaleOffset {
    float scale;
    int32_t offset;
};

struct QnnAxisScaleOffset {
    int32_t axis;
    uint32_t numScaleOffsets;
    QnnScaleOffset* scaleOffset;
};

struct QnnBwScaleOffset {
    uint32_t bitwidth;
    float scale;
    int32_t offset;
};
...
using AHardwareBufferGetNativeHandleFn =
    const NativeHandlePrefix* (*)(const AHardwareBuffer*);
...
bool symbolComesFromHtp(const void* symbol) {
    ...
    return path.find("libqnnhtp.so") != std::string::npos;
}
...
bool writeInputAhb(AhbBuffer& input, const int8_t (&values)[4], std::string& failure) {
    ...
    if (AHardwareBuffer_lock(input.buffer, usage, -1, nullptr, &address) != 0 || address == nullptr) {
        failure = "ahb-input-lock-failed";
        return false;
    }
    std::memcpy(address, values, sizeof(values));
    const int unlockResult = AHardwareBuffer_unlock(input.buffer, nullptr);
    if (unlockResult != 0) {
...
bool verifyOutputAhb(AhbBuffer& output, std::string& failure) {
    ...
    if (AHardwareBuffer_lock(output.buffer, usage, -1, nullptr, &address) != 0 || address == nullptr) {
        failure = "ahb-output-lock-failed";
        return false;
    }

    const auto* values = static_cast<const int8_t*>(address);
    const int8_t expected[4] = {0, 0, 3, 12};
    const bool matches = std::memcmp(values, expected, sizeof(expected)) == 0;
    const int unlockResult = AHardwareBuffer_unlock(output.buffer, nullptr);
    if (unlockResult != 0) {
...
QnnHtpSmokeResult probeQnnHtpGraphAndSharedMemory(void* qnnHtpHandle) noexcept {
    QnnHtpSmokeResult result{};

#ifdef __ANDROID__
    if (qnnHtpHandle == nullptr) {
    ...
    const auto cleanup = [&]() noexcept {
        if (api != nullptr && api->memDeRegister != nullptr) {
            if (outputMem != nullptr) {
                QnnHandle handle = outputMem;
                api->memDeRegister(&handle, 1);
                outputMem = nullptr;
            }
            if (inputMem != nullptr) {
                QnnHandle handle = inputMem;
                api->memDeRegister(&handle, 1);
                inputMem = nullptr;
            }
        }
    ...
    try {
        auto getProviders = reinterpret_cast<GetProvidersFn>(
            dlsym(qnnHtpHandle, "QnnInterface_getProviders"));
        if (getProviders == nullptr || !symbolComesFromHtp(
                reinterpret_cast<const void*>(getProviders))) {
            result.failureReason = "qnn-htp-provider-origin-unverified";
            cleanup();
            return result;
        }
        ...
        if (getProviders(&providers, &providerCount) != QNN_SUCCESS
            ...
            result.failureReason = "qnn-htp-provider-enumeration-failed";
            cleanup();
            return result;
        }
        ...
        result.htpAttributionQualified = true;
        ...
        if (api->backendCreate(nullptr, nullptr, &backend) != QNN_SUCCESS || backend == nullptr) {
            result.failureReason = "qnn-htp-backend-create-failed";
            cleanup();
            return result;
        }
        ...
        if (api->deviceCreate != nullptr) {
            ...
            if (api->deviceCreate(nullptr, nullptr, &candidate) == QNN_SUCCESS)
                device = candidate;
        }

        if (api->contextCreate(backend, device, nullptr, &context) != QNN_SUCCESS
                || context == nullptr) {
            result.failureReason = "qnn-htp-context-create-failed";
            cleanup();
            return result;
        }
        ...
        if (api->memRegister(context, &inputDescriptor, 1, &inputMem) != QNN_SUCCESS
                || inputMem == nullptr) {
            result.failureReason = "qnn-ahb-input-mem-register-failed";
            cleanup();
            return result;
        }
        if (api->memRegister(context, &outputDescriptor, 1, &outputMem) != QNN_SUCCESS
                || outputMem == nullptr) {
        ...
        if (api->graphCreate(context, "lsfg_phase2_htp_smoke", nullptr, &graph) != QNN_SUCCESS
                || graph == nullptr) {
        ...
        if (api->graphExecute(graph, &executeInput, 1, &executeOutput, 1,
