...
constexpr uint64_t FRAMEGEN_COMPLETION_TIMEOUT_NS = 500'000'000ULL;
...
VkResult LsContext::present(const Hooks::DeviceInfo& info, const void* pNext, VkQueue queue,
    ...
    const bool framegenReady = conf.performance
        ? LSFG_3_1P::waitContext(*this->lsfgCtxId, FRAMEGEN_COMPLETION_TIMEOUT_NS)
        : LSFG_3_1::waitContext(*this->lsfgCtxId, FRAMEGEN_COMPLETION_TIMEOUT_NS);
    ...
    if (!framegenReady) {
        ...
        std::cerr << "lsfg-vk: runtime stage=framegen-completion-timeout timeout_ms="
                  << (FRAMEGEN_COMPLETION_TIMEOUT_NS / 1'000'000ULL)
