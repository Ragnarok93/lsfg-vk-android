...
int main() {
    ...
    // Fixed mode still derives final cadence from source budget * multiplier.
    ...
    // Without an explicit source limiter there is no authoritative Adaptive
    // target and no idle budget to spend inside vkQueuePresentKHR.
    assert(resolveOutputPacingTarget(false, 60, 0, 2) == 0);
    assert(resolveOutputPacingTarget(true, 120, 0, 2) == 0);

    // Adaptive is strictly pegged to the GameNative limiter. A stale legacy
    // adaptiveTargetFps value cannot move output away from sourceFpsLimit.
    assert(resolveOutputPacingTarget(true, 60, 30, 2) == 30);
    assert(resolveOutputPacingTarget(true, 70, 30, 2) == 30);
    assert(resolveOutputPacingTarget(true, 120, 30, 2) == 30);
    assert(resolveOutputPacingTarget(true, 70, 30, 3) == 30);
    assert(resolveOutputPacingTarget(true, 120, 30, 4) == 30);
