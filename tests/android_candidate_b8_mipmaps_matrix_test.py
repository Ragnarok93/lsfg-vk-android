...
def main() -> None:
    ...
    assert profile_start < diagnostic_start < b6_index < invocation_index < constants_index < profile_end, (
        "B6/B8 executable capture and matrix transforms must remain explicit B8 opt-in diagnostics "
        "inside profiling selection; normal profiling builds must retain the B4/B11 runtime path"
    )
