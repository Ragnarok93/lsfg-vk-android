...
class AndroidCandidateB4Beta4PredicateContractTest(unittest.TestCase):
    def test_beta4_predicate_transform_is_exact_fingerprinted_and_semantics_preserving(self) -> None:
        ...
        required = (
            ...
            '0x7887fee01585449eULL',
            '49984',
            'kB4ExpectedBound = 2311',
            'kB4PredicateCount = 5',
        ...
        # B4 may redirect only branch predicates. It must not modify the algorithmic
        # hot path: barriers, image sampling/writes, formats, workgroup size, or ICB mode.
        forbidden = (
            'OpControlBarrier)',
        ...
        # Non-profiling Android builds still receive B4 after Candidate-B has
        # supplied the shader name; the optimization must not depend on B3.
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            trans = root / 'src/extract/trans.cpp'
            trans.parent.mkdir(parents=True)
        ...
        self.assertLess(
            production.index('applyCandidateB4Beta4PredicateCanonicalization(shaderName, spirvBytecode);'),
