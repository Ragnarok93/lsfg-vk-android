...
namespace {
    ...
    void selectConstructionFlowScale(float flowScale) {
        ...
        // Context shader/resource objects retain shared ownership of their
        // buffers, samplers, descriptor sets, and images. Replacing the small
        // construction-time resource cache therefore cannot invalidate contexts
        // already created for another Adaptive Flow state. ShaderPool remains
        // untouched so all states reuse compiled modules/pipelines.
...
int32_t LSFG_3_1::createContextFromAHBAtFlowScale(
        AHardwareBuffer* in0, AHardwareBuffer* in1,
        const std::vector<AHardwareBuffer*>& outN,
        VkExtent2D extent, VkFormat format, float flowScale) {
    const std::scoped_lock lock(runtimeMutex);
