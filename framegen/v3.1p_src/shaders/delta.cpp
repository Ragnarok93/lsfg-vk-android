...
void Delta::Dispatch(const Core::CommandBuffer& buf, uint64_t frameCount,
    ...
    pass.sixthDescriptorSet.at(frameCount % 3).bind(
        buf, this->pipelines.at(5), this->dynamicInterpolationPhases, timestampOffset);
    buf.dispatch(threadsX, threadsY, 1);
    ...
    pass.descriptorSets.at(7).bind(buf, this->pipelines.at(9), this->dynamicInterpolationPhases, timestampOffset);
    buf.dispatch(threadsX, threadsY, 1);
}
