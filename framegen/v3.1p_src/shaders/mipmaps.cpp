...
Mipmaps::Mipmaps(Vulkan& vk,
    ...
    this->b10Enabled = Optimizations::AdrenoB10::isRuntimeSupported(vk.device);
#endif
...
void Mipmaps::Dispatch(const Core::CommandBuffer& buf, uint64_t frameCount) {
    ...
    // Preserve the original path exactly for all devices that do not pass the
    // B10 Adreno 6xx runtime gate, even when B10 support is compiled in.
