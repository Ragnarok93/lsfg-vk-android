...
void LSFG_3_1P::presentContextWithPhases(int32_t id, int inSem,
...
int32_t LSFG_3_1P::createAdaptiveContextFromAHB(
        AHardwareBuffer* in0, AHardwareBuffer* in1,
        const std::vector<AHardwareBuffer*>& outN,
        VkExtent2D extent, VkFormat format,
...
void LSFG_3_1P::requestContextFlowScale(int32_t id, float flowScale) {
    const std::scoped_lock lock(runtimeMutex);
    if (!instance.has_value() || !device.has_value())
        throw LSFG::vulkan_error(VK_ERROR_INITIALIZATION_FAILED, "LSFG not initialized");
    auto it = contexts.find(id);
...
LSFG::AdaptiveFlowContextState LSFG_3_1P::getContextFlowScaleState(int32_t id) {
    const std::scoped_lock lock(runtimeMutex);
    if (!instance.has_value() || !device.has_value())
        return {};
    auto it = contexts.find(id);
...
LSFG::AdaptiveFlowGpuTiming LSFG_3_1P::getContextGpuTiming(int32_t id) {
    const std::scoped_lock lock(runtimeMutex);
    if (!instance.has_value() || !device.has_value())
        return {};
    auto it = contexts.find(id);
